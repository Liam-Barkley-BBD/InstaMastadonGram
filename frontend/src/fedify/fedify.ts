...
export class FedifyHandler {
  ...
  private async resolveUserFromUrl(
    userUrl: string
  ): Promise<Follower | Following | null> {
    try {
      ...
      return {
        id: userObject.id,
        username:
          userObject.preferredUsername || this.extractUsernameFromUrl(userUrl),
        displayName:
          userObject.name ||
          userObject.preferredUsername ||
          this.extractUsernameFromUrl(userUrl),
  ...
  getFollowingPosts = async (
    ...
    try {
      ...
      while (hasMore && allFollowing.length < maxFollowingToFetch) {
        const followingResponse = await this.getFollowingPaginated(
          handle,
          uri,
          page,
          50
      ...
      const sourceCounts: {
        [key: string]: { count: number; displayName: string; error?: string };
      } = {};
      ...
      for (let i = 0; i < shuffledFollowing.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (following) => {
          try {
            ...
            console.warn(
              `Failed to fetch posts for ${following.username}:`,
              error
            );
            sourceCounts[following.username || following.displayName] = {
              count: 0,
  ...
  getPostsPaginated = async (
    ...
    try {
      const baseQuery = this.buildQueryParams(
        handle,
        `${uri}/outbox?cursor=0&page=true`
      );
      ...
      return {
        ...
        hasNextPage:
          posts.length === limit &&
          page * limit < (postsResponse.totalItems || 0),
    ...
    try {
      ...
      if (orderedItems.length > 0) {
        if (typeof orderedItems[0] === "string") {
          ...
          followers = orderedItems.map((follower: any) => ({
            id: follower.id,
            username:
              follower.preferredUsername || follower.name?.split("@")[0],
      ...
      return {
        ...
        hasNextPage:
          ...
          page * limit < (followersResponse.totalItems || 0),
    ...
    try {
      ...
      return {
        ...
        hasNextPage:
          ...
          page * limit < (followingResponse.totalItems || 0),
  ...
  private async resolveMultiplePosts(
    postUrls: string[],
    maxConcurrent: number = 5
  ): Promise<Post[]> {
    const results: Post[] = [];
  ...
  followUser = async (handle?: string, uri?: string) => {
    ...
    return await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/users/${handle}/activity`,
  ...
  unfollowUser = async (handle?: string, uri?: string) => {
    ...
    return this.makeRequest(
      `${this.API_BASE_URL}/users/unfollow${query}`,
      {
        method: "DELETE",
      },
      0
  ...
  searchUsers = async (searchText: string) => {
    return this.makeRequest(
      `${this.API_BASE_URL}/users/search?query=${encodeURIComponent(
        searchText
      )}`,
      {},
      0
  ...
  isFollowing = async (handle?: string, uri?: string) => {
    ...
    try {
      const query = this.buildQueryParams(handle, uri);
      const following = await this.makeRequest(
        `${this.API_BASE_URL}/users/following${query}`,
        {},
        0,
        true
      );
      ...
      return following.some(
        (user: any) => user.handle === targetHandle || user.url === uri
      );
    } catch (error) {

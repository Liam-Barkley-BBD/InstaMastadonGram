...
export class FedifyHandler {
      ...
      if (orderedItems.length > 0) {
        if (typeof orderedItems[0] === "string") {
          ...
          posts = orderedItems.map((post: any) => ({
            id: post.id,
            content:
              this.stripHtml(post.object?.content || post.content || "") ||
  ...
  followUser = async (handle: string) => {
    return this.makeRequest(
      `${this.API_BASE_URL}/users/${handle}/follow`,
      {
        method: "POST",
      },
      0
  ...
  unfollowUser = async (handle: string) => {
    return this.makeRequest(
      `${this.API_BASE_URL}/users/${handle}/unfollow`,
      {
        method: "DELETE",
      },
      0
  ...
  getAllPosts = async (
    ...
    try {
      const response = await this.makeRequest(
        `${this.EXPRESS_URL}/posts/all?page=${page}&limit=${limit}`,
        {},
        0,
        false
      );
  ...
  isFollowing = async (handle: string) => {
    try {
      const following = await this.makeRequest(
        `${this.API_BASE_URL}/users/${handle}/following`,
        {},
        0,
        true
      );
